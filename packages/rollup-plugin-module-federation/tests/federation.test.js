...
describe('getFederatedImportStatementForNode', () => {
  ...
  test('should handle ImportDeclaration with ImportDefaultSpecifier', () => {
    ...
    const result = getFederatedImportStatementForNode(node, { importStmt, entityToImport }, 'remote');
  ...
  test('should handle ImportDeclaration with ImportNamespaceSpecifier', () => {
    const node = {
      type: 'ImportDeclaration',
      specifiers: [{ type: 'ImportNamespaceSpecifier', local: { name: 'ABC' } }],
    };
    const result = getFederatedImportStatementForNode(node, { importStmt, entityToImport }, 'remote');
  ...
  test('should handle ImportDeclaration with ImportSpecifier', () => {
    const node = {
      type: 'ImportDeclaration',
      specifiers: [{ type: 'ImportSpecifier', local: { name: 'ABC' }, imported: { name: 'XYZ' } }],
    };
    const result = getFederatedImportStatementForNode(node, { importStmt, entityToImport }, 'remote');
  ...
  test('should handle ExportNamedDeclaration', () => {
    const node = {
      ...
      specifiers: [{ type: 'ExportSpecifier', local: { name: 'ABC' }, exported: { name: 'XYZ' } }],
    };
    const result = getFederatedImportStatementForNode(node, { importStmt, entityToImport }, 'remote');
    expect(result).toContain(`const { ABC } = (await import('entity')); export { ABC as XYZ }`);
  });
});